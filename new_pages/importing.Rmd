...
-   An RDS file (.rds) stores a single R object such as a data frame. These are useful to store cleaned data, as they maintain R column classes. RDSファイル（.rds）は、データフレームなどの単一のRオブジェクトを格納します。これらはR列クラスを維持するため、クリーンアップされたデータを格納するのに役立ちます。Read more in [tこちらのセクションhis section](#import_rds).\
