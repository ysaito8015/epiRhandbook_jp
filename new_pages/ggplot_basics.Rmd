...
+-------------------------------------+--------------------------------------------------------------------------------+
| `plot.caption = element_text()`     | キャプション（ファミリー、フェイス、カラー、サイズ、アングル、vjust、hjust...) |
+-------------------------------------+--------------------------------------------------------------------------------+
...
完全なリストである[テーマの変更](https://ggplot2.tidyverse.org/reference/theme.html)に関する **tidyverse** ドキュメント。

***TIP*****:** **ggplot2** から `theme_get()` を実行すると、90 以上の `theme()` 引数のリストがコンソールに表示されます。

***TIP:*** もしプロットの要素を削除したくなったら、`theme()` を通して行うこともできます。`element_blank()` を引数に渡せば、その要素は完全に消えてしまいます。凡例については、`legend.position = "none"` と設定します。
...
ヒストグラムは棒グラフに似ていますが、[連続変数]{.ul}の分布を測定するという点で異なります。"棒" の間にはスペースがなく、`geom_histogram()` には1つの列しか与えられません。以下は、**ヒストグラム**を生成するためのコードです。ヒストグラムは、連続データを範囲に分け、高さの異なる隣り合った棒で表示します。これは `geom_histogram()` を使って行われます。`geom_histogram()` 、`geom_bar()` 、`geom_col()` の違いについては、ggplot の基本ページの ["棒グラフ" の項](#ggplot_basics_bars)を参照してください。
