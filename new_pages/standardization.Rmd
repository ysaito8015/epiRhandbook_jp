...
このコードチャンクでは解析に必要なパッケージの読み込みを示しています。このハンドブックでは、**pacman** の `p_load()` を重視しています。この関数は必要な場合はパッケージをインストールして*かつ*読み込みも行ってくれます。パッケージのインストールは R **base** の `library()` でも行うことができます。R パッケージに関する詳細な情報については、[R の基礎](#basics) のページを参照してください。 
...
<span style="color: orange;">**_注意：_** `str_replace_all()` を使用してプラス*記号*を削除しようとしても、プラス記号が特別な記号なためうまくいきません。特別な記号の前にバックスラッシュ（Windows では &yen; 記号）を2つ入れて"エスケープ"する必要があります。例：`str_replace_call(column, "\\+", "")`。</span>
